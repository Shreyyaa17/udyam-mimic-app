...
// GET /api/udyam/register - Test endpoint
router.get("/register", (req, res) => {
  res.json({
    success: true,
    message: "Registration endpoint ready",
    method: "POST",
    fields: ["aadhaar", "entrepreneur_name", "enterprise_name", "pan", "email"],
  });
});

// POST /api/udyam/register - Submit registration
router.post("/register", FormController.submitUdyamRegistration);
...
// Additional routes without wildcards
router.get("/search", FormController.searchApplications);
router.get("/stats", FormController.getApplicationStats);
...
router.post("/validate/aadhaar", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      available: !exists,
      message: exists ? "Aadhaar already registered" : "Aadhaar available",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
